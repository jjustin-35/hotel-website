  ...
  const onsubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${config.API_URL}/api/v1/user/signup`,
        form
      );
    ...
    <div className="loginForm">
        ...
        <div>
          <ol className="stepper px-0">
            <li className="stepper_item active">
              <h3 className="stepper_num mb-2" ref={step1}>
                {" "}
                1{" "}
              </h3>
              <i
                className="bi bi-check-lg stepper_num mb-2 completed d-none"
                ref={stepCompleted}
              ></i>
      ...
      <form onSubmit={handleSubmit(onsubmit)}>
        <div className="formStep1" ref={formStep1}>
          ...
          <button
            type="button"
            className="btn w-100 py-3 rounded mb-3 btn-login"
            style={{ background: "white" }}
            disabled={!email || !password || !checked || password !== checked}
            onClick={() => {
              nextStep();
            }}
          >
        ...
        <div className="formStep2 d-none" ref={formStep2}>
            ...
            <div className="row mb-3 g-2">
              ...
              <div className="col-6">
                ...
                >
                  ...
                  {addressData
                    .find((city) => city.CityName === chosenCity)
                    ?.AreaList?.map((area) => {
                      return (
                        <option value={area.ZipCode} key={area.AreaName}>
                          {area.AreaName}
                        </option>
                      );
                    })}
          ...
          <button
            type="button"
            className="btn btn-primary text-white rounded w-100 mb-3"
            onClick={() => prevStep()}
          >
            上一步
          </button>
