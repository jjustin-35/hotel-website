...
export const Input:FC<InputObj> = ({ id, labelText, register, type, errors, rules, placeholder,action }) => {
    return (
      <>
        <label htmlFor={id} className={`form-label text-white ${!!labelText ?'d-block':'d-none'}`}>
          {labelText}
        </label>
        <input
          id={id}
  ...
  export const CheckboxRadio:FC<CheckboxRadioObj> = ({click,id, labelText, register, type, errors, rules, value, name}) => {
    return (<>
